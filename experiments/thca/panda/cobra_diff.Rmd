...
cobra <- cobra(X, vsd)
Sigma_D <- cobra$Q %*% diag(cobra$psi[2,]) %*% t(cobra$Q)
...
Sigma_D = r.Sigma_D
cache=True
ppi = pd.read_csv("data/thca/panda_cobra/tissues_ppi.txt", sep = '\t', header = None)
motif = pd.read_csv("data/thca/panda_cobra/tissues_motif.txt", sep = '\t', header = None)
