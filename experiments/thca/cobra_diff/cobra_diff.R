...
table(dynamicColors)
write.csv(dynamicColors, "./data/thca/cobra_diff/module_assignment.txt")


pdf("./figures/thca/cobra_diff/modules_size.pdf")
#dynamicColors <- read.csv("./data/thca/cobra_diff/module_assignment.txt", row.names = 1)
M <- hashmap()
M[['grey']] <- 'unassigned'
cnt <- 0
...
barplot(table(dynamicColors[,2]), names.arg = c(as.character(seq_len(27)), "unassigned"), ylab = 'Number of genes', main = 'Module sizes after COBRA correction', las = 2, cex.names = .8)
dev.off()
...
for(color in rownames(table(dynamicColors[,1]))){
  if(color != 'grey'){
    pathways_go <- c(pathways_go, 
                         enrichmentGO(rownames(vsd)[dynamicColors[,1] == color], paste("./figures/thca/cobra_diff/GO_", color, ".pdf", sep = "")))
  }
}
...
for(color in rownames(table(dynamicColors[,1]))){
  if(color != 'grey'){
    pathways_kegg <- c(pathways_kegg, 
                         enrichmentKEGG(rownames(vsd)[dynamicColors[,1] == color], paste("./figures/thca/cobra_diff/KEGG_", color, ".pdf", sep = "")))
  }
}
...
data[data == 0] <- NA
pdf("./figures/thca/cobra_diff/GO_summary.pdf", width = 45, height = 45)
...
my_colors <- colorRampPalette(c("lightgrey", "blue"))
data[data == 0] <- NA
pdf("./figures/thca/cobra_diff/KEGG_summary.pdf", width = 45, height = 45)
