...
experiment <- function(samples, index){
  X <- cbind(rep(1, length(samples)), cancer[samples], (sex == 'female')[samples], age[samples], as.matrix(dummy_cols(race)[-1])[samples, ], as.matrix(dummy_cols(stage)[-1])[samples, ], as.matrix(dummy_cols(batch)[-1])[samples, ])
  cobra <- cobra(X, vsd[,samples] + matrix(rnorm(19711 * dim(X)[1], sd=1e-9), 19711, dim(X)[1]))
  Sigma_D <- abs(cobra$Q %*% diag(cobra$psi[2,]) %*% t(cobra$Q))
  rownames(Sigma_D) <- rownames(vsd[,samples])
  colnames(Sigma_D) <- rownames(vsd[,samples])
  
  powers = c(c(1:10), seq(from = 12, to=20, by=2))
  sft = pickSoftThreshold(Sigma_D, powerVector = powers, verbose = 5)
  pdf(paste("./figures/thca/cobra_diff/bootstrap/run", index, "/pre_processing.pdf", sep = ""))
  sizeGrWindow(9, 5)
  par(mfrow = c(1,2));
  cex1 = 0.9;
  ...
  table(dynamicColors)
  write.csv(dynamicColors, paste("./figures/thca/cobra_diff/bootstrap/run", index, "/module_assignment.txt", sep = ""))
  
  pdf(paste("./figures/thca/cobra_diff/bootstrap/run", index, "/modules_size.pdf", sep = ""))
  dynamicColors <- read.csv(paste("./figures/thca/cobra_diff/bootstrap/run", index, "/module_assignment.txt", sep = ""), row.names = 1)
  l <- length(table(dynamicColors))
  M <- hashmap()
  M[['grey']] <- 'unassigned'
  cnt <- 0
  ...
  barplot(table(dynamicColors[,2]), names.arg = c(as.character(seq_len(l-1)), "unassigned"), ylab = 'Number of genes', main = 'Module sizes after COBRA correction', las = 2, cex.names = .8)
  dev.off()
  ...
  for(color in rownames(table(dynamicColors[,1]))){
    if(color != 'grey'){
      pathways_go <- c(pathways_go, 
                       enrichmentGO(rownames(vsd)[dynamicColors[,1] == color], paste("./figures/thca/cobra_diff/bootstrap/run", index, "/GO_", color, ".pdf", sep = "")))
    }
  }
  ...
  for(color in rownames(table(dynamicColors[,1]))){
    if(color != 'grey'){
      pathways_kegg <- c(pathways_kegg, 
                         enrichmentKEGG(rownames(vsd)[dynamicColors[,1] == color], paste("./figures/thca/cobra_diff/bootstrap/run", index, "/KEGG_", color, ".pdf", sep = "")))
    }
  }
  ...
  data[data == 0] <- NA
  pdf(paste0(paste0("./figures/thca/cobra_diff/bootstrap/run", index, "/GO_summary_", index), ".pdf"), width = 45, height = 45)
  gplots::heatmap.2(t(data), scale = "none", Colv=NA, Rowv=NA, col = my_colors(100), revC = TRUE, cexRow = 3, cexCol = 2, margins =c(85,80),
  ...
  my_colors <- colorRampPalette(c("lightgrey", "blue"))
  data[data == 0] <- NA
  pdf(paste0(paste0("./figures/thca/cobra_diff/bootstrap/run", index, "/KEGG_summary_", index), ".pdf"), width = 45, height = 45)
...
set.seed(43)
folds <- sample(c(1, 2, 3, 4, 5), length(sex), replace=T)

for(i in c(1, 2, 3, 4, 5)){
  samples <- seq(1, length(folds), 1)[folds != i]
  samples <- sample(seq_len(length(cancer)), length(cancer), replace=T)
  experiment(samples, i)
}